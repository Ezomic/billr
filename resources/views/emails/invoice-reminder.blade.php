...
<head>
    <meta charset="UTF-8">
    <title>Reminder: invoice {{ $invoice->number }}</title>
</head>
...
<div style="max-width: 600px; margin: 40px auto; background: #ffffff; border-radius: 8px; overflow: hidden; border: 1px solid #e5e7eb;">
    ...
    <div style="padding: 40px;">
        ...
        <p>
            This is a friendly reminder that invoice <strong>{{ $invoice->number }}</strong> is now
            <strong>{{ $daysOverdue }} {{ \Illuminate\Support\Str::plural('day', $daysOverdue) }}</strong> past its due date.
        </p>
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 24px 0; background: #f9fafb; border-radius: 8px; overflow: hidden;">
            ...
            <tr>
                <td style="padding: 12px 16px; color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">Amount Due</td>
                <td style="padding: 12px 16px; font-weight: 600;">{{ number_format($invoice->total / 100, 2) }} {{ $invoice->currency }}</td>
            </tr>
            @if($invoice->due_at)
            <tr>
                <td style="padding: 12px 16px; color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">Was Due</td>
        ...
        <p>A copy of the invoice is attached to this email as a PDF.</p>
        <p style="margin-bottom: 0; color: #6b7280;">If you have already paid, please ignore this reminder.</p>
    </div>
